...
#		self.current_bit = self.current_nib = 0
	...
	def read_string(self, pos = -1, table = None, end_char = 0x00, n_chars = 1000000):
		if pos >= 0:
			self.save_state()
		res = u''
		i = 0
...
#				print '%02X/%02X' % (self.read_b(i), j)
	...
	def dump(self, start = 0, end = -1, tbl = None):
		if end < 0:
			end = len(self)
